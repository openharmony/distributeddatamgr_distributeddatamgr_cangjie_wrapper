...
- 数据共享谓词：面向开发者提供数据查询筛选能力，支持构建复杂的查询条件。包括比较谓词等于、逻辑谓词与、范围谓词包含和介于以及结果集的排序功能（支持递增和递减排序）。当开发者需要在不关注具体数据库类型的情况下检索数据时，数据共享谓词可作为通用的查询条件使用。

- 分布式键值数据库：面向开发者提供跨设备的分布式数据协同能力，支持在多个设备间实现数据的无缝同步与共享。该接口能够将数据持久化保存到分布式键值数据库中，并提供完整的数据操作接口，包括数据的增删改查、端到端同步、数据订阅与通知等核心功能，满足分布式应用对数据一致性和实时性的要求。

- 用户首选项：面向开发者提供轻量级的Key-Value键值型数据处理能力，专门用于应用配置信息和用户偏好的持久化存储。支持对数据的原子性修改和高效查询操作，适用于存储简单的配置参数、用户设置等小规模数据，具有占用资源少、读写速度快的特点。

- 关系型数据库：面向开发者提供一套基于SQLite组件的完整本地数据库管理机制，支持标准的关系型数据模型。提供丰富的数据操作接口，包括数据记录的增删改查等接口，同时也支持直接执行用户自定义的SQL语句，满足复杂业务场景下的数据管理需求。
